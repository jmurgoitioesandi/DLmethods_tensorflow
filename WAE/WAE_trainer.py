...
from utils import save_loss, get_lat_var, read_train_images_numpy, make_plots_GAN_training
PARAMS = cla()
...
train_data = read_train_images_numpy(img_dir=datadir + '/' + dataname,
                                      N_train=n_train,
                                      batch_size=batch_size)

print('\n --- Creating network folder \n')
savefolder = "WAE_slidenum_" + slide_num + f"_zdim_{z_dim}_batchsize_{batch_size}_lambda_{lambda_param}_LR_{learning_rate}_LRsched_{lr_sched}{save_suffix}"
...
def compute_loss(model, x, lambda_param):
  ...
  reconstruction_loss = tf.math.reduce_sum(tf.math.pow(x-x_recons,2),axis=[1,2,3])
  mmd_pen = mmd_penalty(z, z_prior)
  L1_norm_fake = tf.reduce_sum(x_recons)
  return tf.reduce_mean(reconstruction_loss + lambda_param*mmd_pen), tf.reduce_mean(reconstruction_loss), tf.reduce_mean(mmd_pen), L1_norm_fake, encoded_means_sum
...
def train_step(model, x, optimizer, lambda_param):
  ...
  with tf.GradientTape() as tape:
    tape.watch(x)
    loss, reconstruction_loss, KL_div, L1_norm_fake, encoded_means_sum = compute_loss(model, x, lambda_param)
...
for i in range(n_epoch):
    ...
    for true in train_data: #Every "true" is a batch in "train_data"  
        ...
        loss, reconstruction_loss, mmd_pen, L1_norm_fake, encoded_means_sum = train_step(wae_model, true, wae_optim, lambda_param)

        loss_log.append(loss.numpy())
        reconstruction_loss_log.append(reconstruction_loss.numpy())
        mmd_pen_log.append(mmd_pen.numpy())

        L1_norm_true = tf.math.reduce_mean(tf.math.reduce_sum(true, axis=tf.range(1,true.shape.ndims)))
        ...
        if n_iters % 100 == 0:
          print(f"     *** iter:{n_iters} ---> reconstruction_loss:{reconstruction_loss.numpy():.4e}, mmd_pen:{mmd_pen.numpy():.4e}")
        n_iters += 1
    ...
    if (i==0) or ((i+1) % PARAMS.savefig_freq == 0):
        ...
        make_plots_GAN_training(x_generated, n_out, savedir, i, type_im="synth_generative")
        make_plots_GAN_training(x_reconstructed, n_out, savedir, i, type_im="synth_reconstruction")
        make_plots_GAN_training(true[0:5,:,:,:], n_out, savedir, i, type_im="true")
