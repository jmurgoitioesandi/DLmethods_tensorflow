...
def generate_data_MGAN_example_1(interval=(-3.0, 3.0), N_total=100):
    ...
    dataset[:, 1] = np.tanh(dataset[:, 0]) + np.random.gamma(
        shape=1.0, scale=0.3, size=int(N_total * 1.2)
    )

    return dataset[0:N_total, :], dataset[N_total:, :]


def generate_data_MGAN_example_2(interval=(-3.0, 3.0), N_total=100):

    dataset = np.ones((int(N_total * 1.2), 2))
    dataset[:, 0] = np.random.uniform(interval[0], interval[1], int(N_total * 1.2))
    dataset[:, 1] = np.tanh(
        dataset[:, 0] + np.random.normal(0, 0.05, size=int(N_total * 1.2))
    )

    return dataset[0:N_total, :], dataset[N_total:, :]


def generate_data_MGAN_example_3(interval=(-3.0, 3.0), N_total=100):
...
def generate_data_Deep_bimodal_example(interval=(-3.0, 3.0), N_total=100):

    dataset = np.ones((int(N_total * 1.2), 2))
    dataset[:, 0] = np.random.normal(0, 1.0, int(N_total * 1.2))
    dataset[:, 1] = np.cbrt(
        dataset[:, 0] + np.random.normal(0, 1, size=int(N_total * 1.2))
    )
...
def calculate_errors_jointdist(true_X, true_Y, fake):
    ...
    relative_L2_error = tf.norm(hist2d_fake - hist2d_true, ord="euclidean") / tf.norm(
        hist2d_fake, ord="euclidean"
    )
...
def sample_mollified(model, true_X, z, n_total, ball_radius=0.01):
    ...
    epsilon = np.reshape(
        np.random.normal(0, ball_radius, n_total), newshape=true_X.shape
    )
